...
ADMIN_TOKEN="${DSTACK_GATEWAY_ADMIN_TOKEN:-}"
if [ -z "$ADMIN_TOKEN" ]; then
  echo "ERROR: DSTACK_GATEWAY_ADMIN_TOKEN must be set"
  exit 1
fi
...
while [ $retry -lt $max_retries ]; do
  if admin_curl -sf "http://$ADMIN_ADDR/prpc/Status" >/dev/null 2>&1; then
    break
  fi
  retry=$((retry + 1))
  sleep 5
...
admin_curl -sf -X POST "http://$ADMIN_ADDR/prpc/SetCertbotConfig" \
...
if [ -n "$CF_API_TOKEN" ]; then
  existing=$(admin_curl -sf "http://$ADMIN_ADDR/prpc/ListDnsCredentials" 2>/dev/null)
  ...
  if [ "$cred_count" = "0" ]; then
    ...
    admin_curl -sf -X POST "http://$ADMIN_ADDR/prpc/CreateDnsCredential" \
      -H "Content-Type: application/json" \
...
if [ -n "$SRV_DOMAIN" ]; then
  existing=$(admin_curl -sf "http://$ADMIN_ADDR/prpc/ListZtDomains" 2>/dev/null)
  ...
  if [ -z "$has_domain" ]; then
    ...
    admin_curl -sf -X POST "http://$ADMIN_ADDR/prpc/AddZtDomain" \
      -H "Content-Type: application/json" \
