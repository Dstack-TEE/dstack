...
//!
//!   # yamux multiplexed test (single connection, unlimited streams)
//!   stress-client --yamux localhost:4433 --concurrency 100000 --total 100000
...
async fn main() -> Result<()> {
    ...
    let target = match (&args.uds, &args.tcp, &args.yamux) {
        (Some(path), None, None) => Target::Uds(path.clone()),
        (None, Some(addr), None) => Target::Tcp(addr.clone()),
        (None, None, Some(addr)) => Target::Yamux(addr.clone()),
        _ => bail!("specify exactly one of --uds, --tcp, or --yamux"),
    };
    ...
    let mode = if yamux_conn.is_some() {
        "yamux"
    } else {
        "direct"
    };
    ...
    for i in 0..args.total {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let result: Result<()> = async {
                let mut stream = if let Some(ref conn) = yamux_conn {
