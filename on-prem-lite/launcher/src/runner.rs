...
/// Directory the CEK private-key PEM is written to, for skopeo's
/// `--decryption-key`. On tmpfs (/run) so the key never touches persistent disk.
const KEYSET_DIR: &str = "/run/cek";

/// One image-decryption private key (the CEK). `kid` labels it; the `priv_pem` is
/// handed to skopeo, and ocicrypt (native JWE) decrypts with it.
#[derive(Debug, Clone)]
pub struct KeyEntry {
    pub kid: String,
...
/// Write each private key to its own PEM file and return the paths.
...
pub fn write_keyset(keyset: &[KeyEntry]) -> Result<Vec<PathBuf>> {
    ...
    // clear any stale keys from a previous install so old keys don't linger
...
/// the license named); ocicrypt (native JWE) tries each leased private
...
pub fn decrypt_image(
    ...
    if key_files.is_empty() {
        anyhow::bail!("no decryption keys available (fail-closed)");
    }
    ...
    if !status.success() {
        anyhow::bail!(
            "skopeo decrypt failed (exit {}) — no key is the image's recipient, or unauthorized digest",
