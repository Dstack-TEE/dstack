...
<div className="bg-blue-50 p-4 rounded-lg my-4">
  ...
  <pre className="bg-white p-3 rounded text-sm">
    ...
    curl "[https://crt.sh/?q=%.example.com\&output=json](https://crt.sh/?q=%.example.com\&output=json)" | jq '.'

    # Gateway automatic monitoring

    \[monitoring]
    ct\_log\_check = true
    ct\_log\_interval = "1h"
    alert\_webhook = "[https://alerts.example.com/ct](https://alerts.example.com/ct)"
  </pre>
</div>
...
Detailed network configuration instructions can be found in:

* [Deploying Applications](/docs/deployment-guides/deploying-applications) - Application-specific networking
* Gateway configuration in your `gateway.toml` file
* VMM network settings in `vmm.toml`

## Next Steps

* Learn about [deploying applications](/docs/deployment-guides/deploying-applications) with custom networking
* Explore advanced gateway configurations
* Set up monitoring for your network infrastructure

For detailed deployment information, see the [deployment guide](/docs/deployment-guides/deploying-applications).
