...
6. **Run Components:** Start the core components in separate terminals:
   ...
   ```
7. **Deploy Application:** Use the dstack CLI or web interface ([http://localhost:9080](http://localhost:9080)) to deploy your Dockerized application, specifying a `docker-compose.yml`.
...
3. **Deploy via CLI:** Run the dstack CLI to deploy:
   ...
   ```
4. **Verify Deployment:** Access the web UI at [http://localhost:9080](http://localhost:9080) or use API calls to confirm the application is running.
