...
# Candidate whole-disk block devices. A dstack verity volume is a GPT disk with:
#   partition 1 = filesystem data
#   partition 2 = dm-verity superblock + hash tree
#
# Volumes are identified by root hash, not by name or attach order. The host tags
# each verity disk's serial with the root prefix, so the matching disk can be
# opened directly; a missing or wrong tag falls back to trying every disk.
mapfile -t DEVS < <(lsblk -dnro NAME,TYPE 2>/dev/null | awk '$2=="disk"{print "/dev/"$1}')

# Return the Nth partition device path for a whole disk.
partition_n() { # $1 = disk, $2 = 1-based index
    lsblk -nrpo NAME,TYPE "$1" 2>/dev/null |
        awk -v want="$2" '$2=="part"{i++; if (i==want) {print $1; exit}}'
}

# Detect the filesystem only after dm-verity is active, so filesystem metadata is
# read through /dev/mapper and is covered by the measured root.
fs_type() { # $1 = verity-mapped device -> fstype | (empty)
    blkid -o value -s TYPE "$1" 2>/dev/null || true
}

# Probe every candidate disk once, recording "disk data_part hash_part serial".
# The partition table is untrusted, but that is safe: a tampered table just makes
# veritysetup open the wrong data/hash pair, which fails against the measured
# root. Doing this once keeps per-volume matching cheap.
DEV_INFO=()
scan_devices() {
    local dev data hash serial
    if command -v udevadm >/dev/null 2>&1; then
        udevadm settle --timeout=5 2>/dev/null || true
    fi
    for dev in "${DEVS[@]}"; do
        data=$(partition_n "$dev" 1)
        hash=$(partition_n "$dev" 2)
        if ! { [ -b "$data" ] && [ -b "$hash" ]; }; then
            continue
        fi
        serial=$(cat "/sys/block/$(basename "$dev")/serial" 2>/dev/null)
        DEV_INFO+=("$dev $data $hash $serial")
    done
}

# Open $data+$hash as /dev/mapper/$name if it verifies against $root.
# On success, echoes "/dev/mapper/$name <fs_type>".
try_open() { # $1 = data_part, $2 = hash_part, $3 = root, $4 = name
    local data="$1" hash="$2" root="$3" name="$4" fs
    veritysetup open "$data" "$name" "$hash" "$root" >/dev/null 2>&1 || return 1
...
open_volume() { # $1 = root hash, $2 = mapper name
    local root="$1" name="$2" entry dev data hash serial
    for entry in "${DEV_INFO[@]}"; do
        read -r dev data hash serial <<<"$entry"
...
mount_ro() { # $1 = device, $2 = fs_type, $3 = mountpoint
    case "$2" in
        ...
        ext3|ext4) mount -t "$2" -o ro,noload "$1" "$3" 2>/dev/null ;;
        ext2)      mount -t ext2 -o ro "$1" "$3" 2>/dev/null ;;
        "")             mount -o ro "$1" "$3" 2>/dev/null ;;
