...
fn networking_to_proto(n: &Networking) -> pb::NetworkingConfig {
    pb::NetworkingConfig {
        mode: networking_mode_name(n.mode).into(),
        bridge_name: if n.mode == NetworkingMode::Bridge {
            n.bridge.clone()
        } else {
            String::new()
        },
...
mod tests {
    ...
    #[test]
    fn mac_address_index_zero_keeps_existing_derivation() {
        assert_eq!(mac_address_for_vm("vm-123", &[]), "96:b1:d8:b9:08:e6");
        assert_eq!(
...
impl VmConfig {
    pub fn config_qemu(
        ...
        let networks = resolved_networks(&self.manifest, cfg);
        validate_resolved_networks(&networks)?;
        let hostfwd_index = networks
            .iter()
