...
impl KmsRpc for RpcHandler {
    ...
    ///
    /// Deprecated, and no longer called from this tree: guests and KMS-to-KMS
    /// onboarding ([`crate::onboard_service`]) both mint self-issued RA-TLS
    /// certificates now. It exists because the KMS used to pin this CA for mutual TLS,
    /// which left a self-issued certificate with nothing to chain to; client
    /// certificates are verified by the attestation they carry
    /// (`ra_rpc::ratls_client_verifier`), so that CA no longer establishes anything.
    ///
    /// The key it returns authenticates nobody: it is handed to any caller. Retained
    /// so guest images built before the switch keep booting; remove once no supported
    /// guest image calls it.
