...
docker-build() {
    ...
    local build_args="--build-arg SOURCE_DATE_EPOCH=0 --build-arg GIT_REV=$GIT_REV"
    ...
    # Build the image
    docker buildx build $args --output type=docker,name=$name,rewrite-timestamp=true .
...
docker-build "qemu-builder-temp" "" "shared/qemu-pinned-packages.txt"
