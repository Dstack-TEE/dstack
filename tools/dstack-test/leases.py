...
class LeaseJournal:
    ...
    def path(self, lease_id: str) -> Path:
        if not lease_id or any(char not in "abcdefghijklmnopqrstuvwxyz0123456789-" for char in lease_id):
    ...
    def load(self, lease_id: str) -> Lease:
        ...
        value["resources"] = [OwnedResource(**resource) for resource in value.get("resources", [])]
        return Lease(**value)
