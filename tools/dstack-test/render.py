...
def load_plan(root: Path) -> Plan:
    ...
    return Plan(root, index, guide_path, cases, chapter_docs, section_docs, anchors)
...
def render_session(case_id: str, events: list[dict[str, Any]]) -> str:
    parts = [
        '<details class="session"><summary><span class="chevron">›</span> Complete agent session ('
        + str(len(events))
...
def render_case_result(
    ...
    parts.append(render_session(case.id, events))
...
def load_results(
    ...
    results: dict[str, tuple[dict[str, Any], Path]] = {}
    for case in plan.cases:
        path = path_by_id.get(case.id, case.path / "results" / run_id / "result.json")
        if not path.is_file():
