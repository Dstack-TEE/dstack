...
class LocalSimulatorProvider(FixtureProvider):
    ...
    def prepare(
        ...
        start_helper = plan_root / "automation" / "start-simulator.sh"
...
class ExternalCommandProvider(FixtureProvider):
    ...
    def _default_path(self) -> Path | None:
        """Return the provider checked in under fixtures/providers/<name>.py."""
        if self.plan_root is None:
            return None
        candidate = self.plan_root / "fixtures" / "providers" / f"{self.name}.py"
