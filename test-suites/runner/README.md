...
```bash
test-suites/runner/dstack-test run-case \
  --plan path/to/plan \
  --case tc-gw-pp-001 \
  --workdir "$PWD" \
...
```bash
test-suites/runner/dstack-test serve \
...
```bash
test-suites/runner/dstack-test finalize \
  --plan path/to/plan --run-id run-20260723-001 \
  --context run-context.json

test-suites/runner/dstack-test validate \
  --plan path/to/plan --run-id run-20260723-001
```
...
```bash
test-suites/runner/dstack-test package \
  --plan path/to/plan --run-id run-20260723-001 \
  --output report.tar.gz

test-suites/runner/dstack-test render \
  --plan path/to/plan --run-id run-20260723-001 \
  --output report.html
```
