...
def safe_path(root: Path, relative: str, context: str) -> Path:
    ...
    # Containment is enforced immediately below before the path is returned.
    candidate = (root / relative).resolve()  # lgtm[py/path-injection]
    try:
...
def discover_index(root: Path) -> dict[str, Any]:
    """Aggregate distributed suite metadata in directory-name order."""
    # Keep legacy fixture plans readable while repositories migrate. A suite
    # with metadata.json always uses directory discovery and never index.json.
    if not (root / "metadata.json").is_file():
        return load_json(root / "index.json")
    index = load_json(root / "metadata.json")
    chapters: list[dict[str, Any]] = []
    cases_root = root / "cases"
    for chapter_order, chapter_root in enumerate(
        sorted(path for path in cases_root.iterdir() if path.is_dir()), start=1
    ):
        chapter = load_json(chapter_root / "metadata.json")
        chapter.update(
            order=chapter_order,
            path=str(chapter_root.relative_to(root)),
            sections=[],
        )
        for section_order, section_root in enumerate(
            sorted(path for path in chapter_root.iterdir() if path.is_dir()), start=1
        ):
            section = load_json(section_root / "metadata.json")
            section.update(
                order=section_order,
                path=str(section_root.relative_to(root)),
                cases=[],
            )
            for case_order, case_root in enumerate(
                sorted(path for path in section_root.iterdir() if path.is_dir()),
                start=1,
            ):
                case = load_json(case_root / "metadata.json")
                case_path = str(case_root.relative_to(root))
                case.update(
                    order=case_order,
                    path=case_path,
                    spec={"path": f"{case_path}/case.md", "anchor": case["id"]},
                )
                execution = case.get("execution")
                if isinstance(execution, dict):
                    entrypoint = execution.get("entrypoint")
                    if isinstance(entrypoint, str) and "/" not in entrypoint:
                        execution["entrypoint"] = f"{case_path}/{entrypoint}"
...
def load_plan(root: Path) -> Plan:
    ...
    plan_id = check_id(require(index, "id", "metadata.json"), "metadata.json")
    guide = require(index, "guide", "metadata.json")
    ...
    profiles_path = root / "shared" / "fixtures" / "profiles.json"
    ...
    for chapter in sorted_by_order(
        require(index, "chapters", "metadata.json"), "chapters"
    ):
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="test-plan directory containing distributed metadata",
    )
