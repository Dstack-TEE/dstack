...
def is_empty_success(observation: dict[str, Any]) -> bool:
    """Accept protobuf-empty and its canonical JSON `null` representation."""
    json_null_sha256 = hashlib.sha256(b"null").hexdigest()
    return observation["status"] == 200 and (
        observation["body_len"] == 0
        or (
            observation["body_len"] == 4
            and observation["body_sha256"] == json_null_sha256
...
def main() -> int:
    ...
    try:
        ...
        if any(
            not is_empty_success(item) for item in (valid, absent, compatible, repeat)
        ):
            raise AssertionError(
