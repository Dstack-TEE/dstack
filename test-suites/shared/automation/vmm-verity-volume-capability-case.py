...
"""Record whether the complete vmm-auto-restart-backoff-lifecycle controller is available."""
...
CASE_ID = "tc-vmm-volume-008"
CAPABILITY = "vmm-verity-volume-end-to-end-matrix"
FIXTURE_KEY = "vmm_verity_volume_lifecycle"
REQUIRED = [
    "decision_rows",
    "create_row_argv",
    "persisted_manifest_observer_argv",
    "qemu_argv_observer_argv",
    "measurement_observer_argv",
    "guest_dm_verity_observer_argv",
    "content_binding_observer_argv",
    "rejected_state_observer_argv",
    "restart_argv",
    "historical_version_rows",
    "corrected_retry_argv",
...
def main() -> int:
    ...
    source_matrix = (
        (values.get("vmm") or {}).get("test_input", {}).get("verity_volume_matrix", {})
    )
    evidence = {
        "source_matrix_declared": isinstance(source_matrix, dict)
        and bool(source_matrix),
        "source_matrix_fields": sorted(source_matrix)
    ...
    result = {
        ...
        "remarks": "No shared VM, image, service, device, or credential was substituted or mutated.",
    }
