...
class MatrixRun:
    ...
    def __init__(
        ...
        prepare = (
            ...
            / "test-suites/shared/automation/prepare-kms-upgrade-images.py"
        )
        run(
            [
                str(prepare),
    ...
    def deploy(
        ...
        *,
        ...
        # This writes an isolated test configuration, not a reusable secret.
        compose_yaml.write_text(  # lgtm[py/clear-text-storage-sensitive-data]
    ...
    def deploy_gateway(
        ...
        *,
        ...
        ) / ("test-suites/shared/automation/gateway-certificate-observer.py")
        cloudflare_proxy = pathlib.Path(
            json.loads(self.runtime_path.read_text())["repository"]
        ) / ("test-suites/shared/automation/gateway-cloudflare-zone-proxy.py")
        observer_service = ""
        observer_config = ""
        dns_service = ""
        dns_config = ""
...
{"".join(f"      {line}\n" for line in cloudflare_proxy.read_text().splitlines())}"""
        if version == "0.5.11":
            dns_script = (
                ...
                / "test-suites/shared/automation/gateway-upgrade-dns.py"
            )
...
{configs_section}"""
        ...
        with pathlib.Path("/tmp/dstack-kms-upgrade-port-allocation.lock").open(
            ...
            fcntl.flock(allocation_lock, fcntl.LOCK_EX)
            allocated = self.free_ports(5 + int(evidence_observer))
...
{"".join(f"      {line}\n" for line in compatibility_bridge.read_text().splitlines())}"""
    ...
    def deploy_client(
        ...
        *,
        ...
        observer = (
            pathlib.Path(json.loads(self.runtime_path.read_text())["repository"])
            / "test-suites/shared/automation/kms-upgrade-client-observer.py"
        )
        gateway_cache_volume = (
            "      - /run/dstack:/run/dstack-host:ro\n" if native_gateway else ""
        )
        compose_yaml = self.workspace / f"{name}.compose.yml"
        compose_yaml.write_text(
            f"""services:
...
{"".join(f"      {line}\n" for line in observer.read_text().splitlines())}"""
    ...
    def prepare_client_gateway_wireguard(
        ...
        *,
        ...
        url = (
            f"http://127.0.0.1:{client['service_port']}/gateway-wireguard-probe?ip={ip}"
        )
        deadline = time.monotonic() + timeout
