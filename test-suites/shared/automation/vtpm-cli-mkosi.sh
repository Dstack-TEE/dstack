...
SIM_PID=
LOCAL=(--allowed-collateral-host 127.0.0.1)
...
for a in auto ecc rsa; do "$UTIL" tpm-quote --key-algo "$a" --hash-algo none --data "$DATA" --output $ROOT/out/$a.json; "$UTIL" tpm-verify "${LOCAL[@]}" --root-ca "$ROOT_CA" --quote $ROOT/out/$a.json >$ROOT/out/verify-$a.log; done
test "$(stat -c %a $ROOT/out/auto.json)" = 600
reject "$UTIL" tpm-verify --root-ca "$ROOT_CA" --quote $ROOT/out/auto.json; grep -q 'collateral host 127.0.0.1 is not allowed' $ROOT/out/reject.log
...
reject "$UTIL" tpm-verify "${LOCAL[@]}" --root-ca $ROOT/out/root2.pem --quote $ROOT/out/auto.json
jq '.pcr_values[0].value[0] ^= 1' $ROOT/out/auto.json >$ROOT/out/pcr.json; reject "$UTIL" tpm-verify "${LOCAL[@]}" --root-ca "$ROOT_CA" --quote $ROOT/out/pcr.json
jq '.signature = (.signature[0:-2] + "00")' $ROOT/out/auto.json >$ROOT/out/sig.json; reject "$UTIL" tpm-verify "${LOCAL[@]}" --root-ca "$ROOT_CA" --quote $ROOT/out/sig.json
kill "$SIM_PID"; wait "$SIM_PID" 2>/dev/null || true; SIM_PID=
reject "$UTIL" tpm-verify "${LOCAL[@]}" --root-ca "$ROOT_CA" --quote $ROOT/out/auto.json
...
start_tpm "$SEED1"; "$UTIL" tpm-verify "${LOCAL[@]}" --root-ca $ROOT/runtime/mock-roots/tpm-root-ca.pem --quote $ROOT/out/auto.json >/dev/null
python3 - <<'PY'
import json
print(json.dumps({k:True for k in "vtpm_rsa vtpm_ecc quote_auto quote_ecc quote_rsa verify unlisted_host_rejected wrong_root_rejected pcr_rejected signature_rejected network_rejected device_rejected output_atomic retry adjacent_identity permissions".split()},sort_keys=True))
PY
