...
def main() -> None:
    ...
    registry = json.loads((root / "shared/fixtures/profiles.json").read_text())[
        "profiles"
    ]
    cases = plan.cases
    ...
    for case in cases:
        ...
        actions = case.actions_under_test
    ...
    promoted_path = root / "shared/automation/promoted-passing-cases.json"
    ...
    if errors:
        fail("fixture contract audit failed:\n" + "\n".join(errors))
    counts = Counter(case.fixture["profile"] for case in cases if case.fixture)
    print(
        json.dumps(
