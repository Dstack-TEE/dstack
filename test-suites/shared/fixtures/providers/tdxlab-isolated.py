...
def prepare(value: dict[str, Any]) -> dict[str, Any]:
    ...
    dashboard_log_requested = case_id == "tc-gos-observabil-001" or (
        isinstance(actions, list)
    ...
    if identity_matrix_requested:
        ...
        if alternate_provenance.get("git_revision") != selected_image_provenance.get(
            "git_revision"
        ):
            fail(
    ...
    if (
        ...
        helper = settings["repository"] / (
            "test-suites/shared/automation/start-mkosi-kms-fixture.py"
        )
        completed = run(
            [
                str(helper),
