...
def write_gateway_config(
    ...
    *,
    ...
    replacements = {
        ...
        'reserved_net = ["10.0.0.1/32"]': (f'reserved_net = ["{fixture_subnet}.1/32"]'),
...
def prepare(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if requested.get("profile") == "vmm-empty-control-plane":
        ...
        if "Vmm.GetAppEnvEncryptPubKey" in actions:
            ...
            start_simulator = plan_root / "shared/automation/start-simulator.sh"
            stop_simulator = plan_root / "shared/automation/stop-simulator.sh"
        ...
        create_helper = plan_root / "shared/automation/vmm-create-stopped.py"
        crash_qemu = plan_root / "shared/automation/vmm-crash-qemu.py"
        console_log_control = plan_root / "shared/automation/vmm-console-log-control.py"
        web_ui_workflow = plan_root / "shared/automation/vmm-web-ui-workflow.cjs"
        vsock_http = plan_root / "shared/automation/vsock-http.py"
        if (
    ...
    if requested.get("profile") in {
        ...
        for node_id, ports_for_node in enumerate(node_ports, start=1):
            ...
            if case_id == "tc-gw-registrati-002":
                ...
                if probe.returncode or applied_config.read_text(
                    encoding="utf-8"
            ...
            write_gateway_config(
                ...
                recycle_timeout_seconds=(5 if case_id == "tc-gw-registrati-002" else 2),
                enable_debug=not (
