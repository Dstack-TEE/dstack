...
on_exit() {
  local rc=$?
  if (( rc != 0 )); then
    log "upgrade E2E failed; recent infrastructure logs follow"
...
main() {
  ...
  log "running production-compatible KMS/Gateway rolling-upgrade E2E"
  ...
  DSTACK_E2E_PHASE=upgrade compose run --rm --no-deps \
    -e DSTACK_E2E_PHASE=upgrade \
