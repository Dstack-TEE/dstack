...
def wait_status(
    command: list[str], vm_id: str, wanted: str | None, timeout: float = 30
) -> None:
...
def wait_log(log: pathlib.Path, needle: str, minimum: int, timeout: float = 15) -> int:
    ...
    raise AssertionError(
        f"log count for {needle!r} remained {count}, expected {minimum}"
    )
...
def main() -> int:
    ...
    try:
        ...
        steps.append(
            {
                ...
                "status": "PASS",
                "observed": "The case-owned VMM exposed the exact 1/1/2-second, three-retry policy; eligible, never-started, and removing records were isolated.",
            }
        )
        ...
        evidence["restart"] = {
            "automatic_attempt_events": 4,
            "healthy_reset_events": 1,
            "exhausted_events": 1,
            "final_status": "exited",
            "never_started_status": "stopped",
        }
        steps.append(
            {
                "id": f"{CASE_ID}-step-02",
                "status": "PASS",
                "observed": "Injected five QEMU exits: the eligible VM restarted with bounded backoff, reset after its healthy window, exhausted exactly three consecutive retries, and then remained exited without a hot loop; ineligible peers never restarted.",
            }
        )
        ...
        if invalid_start < 400 or not isinstance(listed(list_command), list):
            raise AssertionError(
                "invalid input or adjacent availability boundary failed"
            )
        evidence["boundaries"] = {
            "invalid_start": invalid_start,
            "list_available": True,
            "decision_events_observed": True,
        }
        steps.append(
            {
                "id": f"{CASE_ID}-step-03",
                "status": "PASS",
                "observed": "Structured restart/reset/exhaustion events were observed; invalid VM input failed closed and the public list remained available.",
            }
        )
    except Exception as error:
        failures.append(f"{type(error).__name__}: {error}")
        evidence["failure_diagnostics"] = {
            "vmm_log_tail": log.read_text(errors="replace")[-6000:]
            if log.is_file()
            else "",
            "vm_stderr_tails": {
                vm_id: (pathlib.Path(vmm["run_path"]) / vm_id / "stderr.log").read_text(
                    errors="replace"
                )[-3000:]
                for vm_id in ids
    ...
    artifact = {
        "path": "artifacts/vmm-auto-restart-policy.json",
        "step_id": f"{CASE_ID}-step-02",
        "name": "Automatic restart fault matrix",
        "description": "Candidate policy rows plus case-owned VMM/QEMU crash, event, retry, recovery, isolation, availability, and cleanup observations.",
    }
    ...
    atomic_json(
        result_dir / "result.json",
        {
            ...
            "provisional": False,
            "status": status_value,
            "summary": "12/12 policy rows and the case-owned crash/restart lifecycle passed."
            if not failures
            else failures[0],
            "steps": steps,
            "artifacts": [artifact],
            ...
            "remarks": "Only three immediately registered VMs and their case-owned Supervisor were mutated; no image was built and provider cleanup remains authoritative.",
        },
    )
