...
def listed(command: list[str]) -> list[dict[str, Any]]:
    ...
    if process.returncode:
        raise RuntimeError("prepared list command failed")
...
def main() -> int:
    ...
    vmm = values["vmm"]
    fixture = values.get("vmm_web_ui_deployment", {})
    required = {
        "browser_session_argv",
        "ui_url",
        "health_probe_argv",
        "semantic_form_rows",
        "ui_submit_argv",
        "created_vm_observer_argv",
        "lifecycle_argv",
        "server_error_row_argv",
        "unset_default_observer_argv",
        "keyboard_accessibility_argv",
        "cross_session_probe_argv",
        "cleanup_argv",
    }
    if (
        ...
        raise RuntimeError("complete browser controller absent")
    name = f"{vmm['test_input']['name_prefix']}-web-ui"
    output = result_dir / "artifacts/browser-workflow.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    env = os.environ.copy()
    env.update(
        {
            "DSTACK_BROWSER_WORKFLOW": str(fixture["browser_workflow"]),
            "DSTACK_UI_URL": str(fixture["ui_url"]),
            "DSTACK_UI_VM_NAME": name,
            "DSTACK_UI_IMAGE": str(vmm["test_input"]["image"]),
            "DSTACK_UI_OUTPUT": str(output),
        }
    )
    ...
    try:
        browser = subprocess.run(
            command, text=True, capture_output=True, timeout=240, check=False, env=env
        )
    ...
    try:
        ...
        if rows != EXPECTED_ROWS:
            raise AssertionError(
                f"browser rows mismatch: missing={sorted(EXPECTED_ROWS - rows)}"
            )
        matches = [
            item for item in listed(list_command) if str(item.get("name")) == name
        ]
        if len(matches) != 1:
            raise AssertionError(
                "UI-created VM was not uniquely observable by public list"
            )
        vm = matches[0]
        vm_id = str(vm["id"])
        registry = json.loads(
            pathlib.Path(vmm["test_input"]["created_vms_registry"]).read_text()
        )
        ...
        if vm_id not in registry:
            registry.append(vm_id)
            pathlib.Path(vmm["test_input"]["created_vms_registry"]).write_text(
                json.dumps(registry) + "\n"
            )
        ...
        evidence.update(
            {
                "rows": sorted(rows),
                "defaults": observed.get("defaults"),
        ...
        steps = [
            {
                ...
                "status": "PASS",
                "observed": "A fresh isolated Chromium context loaded the healthy case-owned VMM and observed exact deployment defaults.",
            },
            {
            ...
            },
            {
                ...
                "status": "PASS",
                "observed": "A second browser context had no leaked form state, observed the same public VM by UUID, and the case remained available for cleanup.",
            },
        ]
    ...
    artifact = {
        "path": "artifacts/browser-workflow.json",
        "step_id": f"{CASE_ID}-step-02",
        "name": "VMM web UI browser workflow",
        "description": "Fresh-context defaults, semantic form, keyboard submission, server-error recovery, lifecycle, update, log, cross-session, public-state, and cleanup evidence.",
    }
    # Preserve browser output as the named artifact and a separate controller summary.
    summary_path = result_dir / "artifacts/vmm-web-ui-summary.json"
    atomic_json(summary_path, evidence)
    summary_artifact = {
        "path": "artifacts/vmm-web-ui-summary.json",
        "step_id": f"{CASE_ID}-step-03",
        "name": "Web UI public-state summary",
        "description": "Public UUID/configuration and cleanup correlation for the browser-created VM.",
    }
    available_artifacts = [
        item
        for item in (artifact, summary_artifact)
        if (result_dir / item["path"]).is_file()
    ]
    atomic_json(
        result_dir / "artifacts/manifest.json", {"artifacts": available_artifacts}
    )
    ...
    atomic_json(
        result_dir / "result.json",
        {
            ...
            "evidence": ev,
            "remarks": "One VM was submitted through a fresh headless Chromium context; direct RPCs were used only for final bounded cleanup.",
        },
    )
