...
Verify that every method `GuestApi` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the guest agent's host-facing listener is reachable.
...
whose `service` is `GuestApi`, and the vector set in
...
Resolve the guest agent's host-facing listener from the lease-owned fixture, load the inventory
entries for `GuestApi`, and confirm every one of them carries a policy.
