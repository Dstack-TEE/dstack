...
<a id="tc-gos-contract-002"></a>
# TC-GOS-CONTRACT-002: DstackGuest request-contract matrix
...
- Automation: Yes
- Requirements: [req-gos-contract-002](../../../../catalog/feature-audit.md#req-gos-contract-002)
- Risks: [risk-gos-contract-002](../../../../catalog/feature-audit.md#risk-gos-contract-002)
...
Verify that every method `DstackGuest` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the guest agent's `dstack.sock` internal listener, which is bind-mounted into application containers is reachable.
...
whose `service` is `DstackGuest`, and the vector set in
...
<a id="tc-gos-contract-002-step-01"></a>
### Step 1: Resolve the listener and the indexed contract

Resolve the guest agent's `dstack.sock` internal listener, which is bind-mounted into application containers from the lease-owned fixture, load the inventory
entries for `DstackGuest`, and confirm every one of them carries a policy.
...
<a id="tc-gos-contract-002-step-02"></a>
...
<a id="tc-gos-contract-002-step-03"></a>
