...
Verify that every `Tappd` method handles absent, valid, boundary-invalid,
wrong-typed, unknown-field and malformed-framing input, in JSON and protobuf,
without breaking the invariants below.

## Preconditions

1. The guest agent's `tappd.sock` legacy listener is reachable on a lease-owned fixture. The matrix drives
   mutating methods with run-scoped identifiers, so it must never target a
   shared deployment.

## Test Data

The `guest-os` entries in `api-inventory.json` whose `service` is `Tappd`,
...
Resolve the listener from the fixture and load the `Tappd` inventory entries.
