...
<a id="tc-gos-contract-001"></a>
# TC-GOS-CONTRACT-001: Tappd request-contract matrix
...
- Automation: Yes
- Requirements: [req-gos-contract-001](../../../../catalog/feature-audit.md#req-gos-contract-001)
- Risks: [risk-gos-contract-001](../../../../catalog/feature-audit.md#risk-gos-contract-001)
...
Verify that every method `Tappd` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the guest agent's `tappd.sock` legacy listener is reachable.
...
whose `service` is `Tappd`, and the vector set in
...
<a id="tc-gos-contract-001-step-01"></a>
### Step 1: Resolve the listener and the indexed contract

Resolve the guest agent's `tappd.sock` legacy listener from the lease-owned fixture, load the inventory
entries for `Tappd`, and confirm every one of them carries a policy.
...
<a id="tc-gos-contract-001-step-02"></a>
