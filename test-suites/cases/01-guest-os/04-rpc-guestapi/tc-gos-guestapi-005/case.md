...
<a id="tc-gos-guestapi-005"></a>
# TC-GOS-GUESTAPI-005: GuestApi.Shutdown
...
- Automation: Yes
- Requirements: [req-gos-guestapi-005](../../../../catalog/feature-audit.md#req-gos-guestapi-005)
- Risks: [risk-gos-guestapi-005](../../../../catalog/feature-audit.md#risk-gos-guestapi-005)
- Source: `dstack/guest-api/proto/guest_api.proto:143`
...
- Prepared RPC contract: `GuestApi.Shutdown` takes `google.protobuf.Empty` (no fields) and returns `google.protobuf.Empty` (no fields). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../../catalog/api-inventory.json); do not reconstruct it from implementation source.
...
- Simulator shutdown safety: never invoke the host's real `systemctl`. The prepared simulator launcher supplies a case-scoped `systemctl` stub and records invocations in `systemctl.log`. At SIMULATOR level, the required side effect is exactly one recorded `poweroff` dispatch per valid call while the simulator remains available until explicit case cleanup; this does not confirm physical guest shutdown.
...
Verify the complete request, response, authorization, state transition, and error contract of `GuestApi.Shutdown`.
...
<a id="tc-gos-guestapi-005-step-03"></a>
