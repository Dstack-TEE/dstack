...
- Automation: Yes
- Requirements: [req-gos-guestapi-004](../../../../catalog/feature-audit.md#req-gos-guestapi-004)
- Risks: [risk-gos-guestapi-004](../../../../catalog/feature-audit.md#risk-gos-guestapi-004)
...
- Prepared RPC contract: `GuestApi.ListContainers` takes `google.protobuf.Empty` (no fields) and returns `ListContainersResponse` (`containers: Container`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../../catalog/api-inventory.json); do not reconstruct it from implementation source.
...
- The valid call returns `ListContainersResponse` with every documented field and exhibits the documented `ListContainers` state and side effects; extraneous or malformed bodies are ignored without changing the response or state, invalid routing returns a structured transport error, and protected calls reject missing credentials.
