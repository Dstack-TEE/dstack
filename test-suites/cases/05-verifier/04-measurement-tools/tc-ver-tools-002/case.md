...
- Zeroed-header kernels (#1189): images built while dstack normalized the header declare `kernel_header_normalized = true`. After an isolated copy is zeroed the same way and the flag is declared, only RTMR1 changes and it equals an independent Authenticode SHA-384 replay of the shipped file; RTMR1 and RTMR2 are identical at 1, 2, 3, and 8 GiB on QEMU 8.2.2, 9.2.1, and 10.2.1.
