...
# TC-KMS-AUTH-003: Ethereum authorization freshness and domain binding
...
- Automation: Yes
- Requirements: [req-kms-auth-003](../../../../catalog/feature-audit.md#req-kms-auth-003)
- Risks: [risk-kms-auth-003](../../../../catalog/feature-audit.md#risk-kms-auth-003)
...
Verify every Ethereum authorization request is freshly evaluated against the configured contract domain, with payload mutations, backend interruption, process restart, and isolation observations.
...
Submit valid and altered app/KMS authorization payloads, contract address and chain-domain configurations, duplicate requests, and repeated requests after a changed backend decision.

**Expected results:**

- Every request invokes the configured contract again; changed measurements or identities produce distinct contract arguments, and an earlier allow is never replayed after the backend decision changes.
...
Restart the stateless authorization process, re-run the decision matrix, and check that no authorization decision or request identity persists across processes.
