...
<a id="tc-kms-upgrade-007"></a>
# TC-KMS-UPGRADE-007: 0.5.4 age-matched ACPI diagnosis
...
- Automation: Yes
- Requirements: [req-kms-upgrade-007](../../../../catalog/feature-audit.md#req-kms-upgrade-007)
- Risks: [risk-kms-upgrade-007](../../../../catalog/feature-audit.md#risk-kms-upgrade-007)
...
Prove a retained 0.5.4 quote is reproducible with the age-matched ACPI tool and explicitly verify whether the candidate ACPI tool remains compatible for the recorded QEMU configuration.
...
<a id="tc-kms-upgrade-007-step-01"></a>
...
<a id="tc-kms-upgrade-007-step-02"></a>
### Step 2: Execute the compatibility path

Run `dstack-mr diagnose` on the same 0.5.4 quote and `vm_config` with both the age-matched and candidate `dstack-acpi-tables`. Use a static diagnosis CLI inside each versioned container so the ACPI tool keeps its matching QEMU ROM environment.

**Expected results:**

- The age-matched tool reports RTMR0 MATCH. Record the candidate result without assuming divergence: for the validated QEMU 8.2.2 configuration both tools reproduce the same RTMR0, despite having different binary digests. Any future mismatch must report the first divergent ACPI event and must not be accepted as upgrade evidence.

<a id="tc-kms-upgrade-007-step-03"></a>
...
<a id="tc-kms-upgrade-007-step-04"></a>
