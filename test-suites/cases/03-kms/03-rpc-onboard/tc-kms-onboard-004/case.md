...
<a id="tc-kms-onboard-004"></a>
# TC-KMS-ONBOARD-004: Onboard.Finish
...
- Automation: Yes
- Requirements: [req-kms-onboard-004](../../../../catalog/feature-audit.md#req-kms-onboard-004)
- Risks: [risk-kms-onboard-004](../../../../catalog/feature-audit.md#risk-kms-onboard-004)
- Source: `dstack/kms/rpc/proto/kms_rpc.proto:173`
...
- Prepared RPC contract: `Onboard.Finish` takes `google.protobuf.Empty` (no fields) and returns `google.protobuf.Empty` (no fields). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../../catalog/api-inventory.json); do not reconstruct it from implementation source.
...
- `Onboard.Finish` is destructive by design: a successful call gracefully closes the onboarding listener and transitions the same process toward the initialized KMS service. Never call it during Step 1 baseline. Run invalid-route checks before Finish, invoke Finish exactly once per target instance, and provision a fresh isolated target before testing another JSON/protobuf/body-ignored representation. Do not grade post-Finish connection refusal as a failed response when the triggering request itself returned HTTP 200 Empty.
...
Verify the complete request, response, authorization, state transition, and error contract of `Onboard.Finish`.
