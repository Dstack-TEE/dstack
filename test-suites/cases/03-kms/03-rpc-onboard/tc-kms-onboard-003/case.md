...
<a id="tc-kms-onboard-003"></a>
# TC-KMS-ONBOARD-003: Onboard.GetAttestationInfo
...
- Automation: Yes
- Requirements: [req-kms-onboard-003](../../../../catalog/feature-audit.md#req-kms-onboard-003)
- Risks: [risk-kms-onboard-003](../../../../catalog/feature-audit.md#risk-kms-onboard-003)
- Source: `dstack/kms/rpc/proto/kms_rpc.proto:171`
...
- Prepared RPC contract: `Onboard.GetAttestationInfo` takes `google.protobuf.Empty` (no fields) and returns `AttestationInfoResponse` (`device_id: bytes`, `mr_aggregated: bytes`, `os_image_hash: bytes`, `tee_variant: string`, `site_name: string`, `eth_rpc_url: string`, `kms_contract_address: string`, `ppid: bytes`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../../catalog/api-inventory.json); do not reconstruct it from implementation source.
...
Verify the complete request, response, authorization, state transition, and error contract of `Onboard.GetAttestationInfo`.
...
- The valid call returns `AttestationInfoResponse` with every documented field and exhibits the documented `GetAttestationInfo` state and side effects; extraneous or malformed bodies are ignored without changing the response or state, invalid routing returns a structured transport error, and protected calls reject missing credentials.

<a id="tc-kms-onboard-003-step-03"></a>
