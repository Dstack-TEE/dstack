...
<a id="tc-kms-contract-001"></a>
# TC-KMS-CONTRACT-001: KMS request-contract matrix
...
- Automation: Yes
- Requirements: [req-kms-contract-001](../../../../catalog/feature-audit.md#req-kms-contract-001)
- Risks: [risk-kms-contract-001](../../../../catalog/feature-audit.md#risk-kms-contract-001)
...
Verify that every method `KMS` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the KMS public RPC listener is reachable.
...
The `kms` entries in [`api-inventory.json`](../../../../catalog/api-inventory.json)
whose `service` is `KMS`, and the vector set in
...
<a id="tc-kms-contract-001-step-01"></a>
### Step 1: Resolve the listener and the indexed contract

Resolve the KMS public RPC listener from the lease-owned fixture, load the inventory
entries for `KMS`, and confirm every one of them carries a policy.
...
<a id="tc-kms-contract-001-step-02"></a>
...
<a id="tc-kms-contract-001-step-03"></a>
