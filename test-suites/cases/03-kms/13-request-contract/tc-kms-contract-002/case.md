...
Verify that every method `Onboard` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the KMS onboarding listener is reachable.
...
whose `service` is `Onboard`, and the vector set in
...
Resolve the KMS onboarding listener from the lease-owned fixture, load the inventory
entries for `Onboard`, and confirm every one of them carries a policy.
