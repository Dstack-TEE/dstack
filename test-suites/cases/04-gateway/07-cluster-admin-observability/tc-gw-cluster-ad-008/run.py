...
def post(
    ...
    try:
        with urllib.request.urlopen(
            request, timeout=5, context=context(identity)
        ) as response:
...
def handshake(
    host: str, port: int, minimum: ssl.TLSVersion, maximum: ssl.TLSVersion
) -> tuple[bool, str, str]:
...
def main() -> int:
    ...
    try:
        ...
        checks["strong_cipher_selected"] = all(
            value[2]
            and not any(word in value[2].upper() for word in ("NULL", "RC4", "DES"))
        ...
        valid_code = post(
            f"{public}/Tproxy.RegisterCvm?json", request, identity=identity
        )[0]
        wrong_dir = pathlib.Path(
            tempfile.mkdtemp(prefix="wrong-client-", dir=result_dir)
        )
        try:
            wrong = generate_wrong_identity(wrong_dir)
            wrong_code = post(
                f"{public}/Tproxy.RegisterCvm?json", request, identity=wrong
            )[0]
        finally:
        ...
        steps = [
            {
                ...
                "status": "PASS",
                "observed": "The case-owned public listener negotiated TLS 1.2 and TLS 1.3 with non-null modern ciphers.",
            },
            {
                "id": f"{CASE_ID}-step-02",
                "status": "PASS",
                "observed": "Plaintext on the public TLS port and TLS on the plaintext admin port failed while authenticated admin HTTP remained available.",
            },
            {
                "id": f"{CASE_ID}-step-03",
                "status": "PASS",
                "observed": "Simulator mTLS registration succeeded; missing and unrelated self-signed client identities failed closed.",
            },
        ]
        observation = {
            "checks": checks,
        ...
        artifacts.append(
            {
                "path": "artifacts/gateway-tls-matrix.json",
                "step_id": f"{CASE_ID}-step-02",
                "name": "Gateway TLS matrix",
                "description": "Negotiated protocol/cipher names, HTTP statuses, and booleans only; no certificate, key, token, URL, or response body is retained.",
            }
        )
        status = "PASS"
    ...
    atomic_json(
        result_dir / "result.json",
        {
            ...
            "steps": steps,
            "artifacts": artifacts,
            "remarks": "Ephemeral wrong-client material was deleted before result publication; no key, certificate, token, URL, or native response body is retained.",
        },
    )
