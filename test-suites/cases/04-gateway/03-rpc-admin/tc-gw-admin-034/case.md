...
<a id="tc-gw-admin-034"></a>
# TC-GW-ADMIN-034: Admin.RemoveCvm
...
- Automation: Yes
- Requirements: [req-gw-admin-034](../../../../catalog/feature-audit.md#req-gw-admin-034)
- Risks: [risk-gw-admin-034](../../../../catalog/feature-audit.md#risk-gw-admin-034)
...
- The authoritative wire contract is the `Admin.RemoveCvm` entry in [`api-inventory.json`](../../../../catalog/api-inventory.json).
...
Verify the complete request, response, authorization, idempotency, failure-atomicity, and operator-recovery contract of `Admin.RemoveCvm`.
...
Exercise `instance_id` empty, malformed, unknown, active, rejected, and repeated-removal values, unknown protobuf fields, JSON and protobuf encodings, and missing admin credentials. The deterministic smoke payload is `{"instance_id":"dstack-test-unknown-instance"}`.

## Steps

<a id="tc-gw-admin-034-step-01"></a>
### Step 1: Verify the baseline and authorization boundary

Query health and bounded admin state, then invoke `Admin.RemoveCvm` without credentials and with malformed framing.
...
<a id="tc-gw-admin-034-step-02"></a>
### Step 2: Exercise valid, absent, boundary, and repeated operations

Invoke `Admin.RemoveCvm` over JSON and protobuf with the case matrix, repeat each idempotent operation, and capture native responses.

**Expected results:**

- Idempotent unknown-instance removal reports no existing record or local mutation; malformed and empty identifiers are rejected.
- JSON and protobuf responses agree structurally, retries converge, and unknown fields do not weaken validation.

<a id="tc-gw-admin-034-step-03"></a>
