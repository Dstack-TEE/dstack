#!/usr/bin/env python3
"""Deterministic contract test for the lease-owned Debug.RegisterCvm RPC."""
...
def inventory(root: pathlib.Path) -> dict[str, Any]:
    """Load the authoritative Debug.RegisterCvm field matrix."""
    document = json.loads((root / "catalog" / "api-inventory.json").read_text())
    found: list[dict[str, Any]] = []

    def walk(value: Any) -> None:
        if isinstance(value, dict):
            if value.get("service") == "Debug" and value.get("method") == "RegisterCvm":
                found.append(value)
    ...
    walk(document)
    if len(found) != 1:
        raise RuntimeError("expected one Debug.RegisterCvm inventory entry")
    return found[0]
...
def main() -> int:
    ...
    base = str(manifest["values"]["gateway"]["debug_url"]).rstrip("/")
    route = f"{base}/Debug.RegisterCvm"
    contract = inventory(plan_root)
    tag = hashlib.sha256(
        f"{manifest['run_id']}:{manifest['lease_id']}:{case_id}".encode()
    ).hexdigest()[:16]
    key = wg_public_key()
    ...
    try:
        ...
        invalid_route = request(route + "NoSuch", b"{}")
        empty = request(route, b"{}")
        if invalid_route["status"] < 400 or empty["status"] < 400:
            raise AssertionError(
        ...
        record(
            ...
            step,
            {"invalid_route": public(invalid_route), "empty": public(empty)},
        ...
        steps.append(
            {
                "id": step,
                "status": "PASS",
                "observed": "The listener rejected invalid routing and an empty registration.",
            }
        )
        ...
        valid_payload = {
            ...
            "app_id": f"app-{tag}",
            "instance_id": f"instance-json-{tag}",
        }
        valid = request(
            route, json.dumps(valid_payload, separators=(",", ":")).encode()
        )
        ...
        wrong_type = request(
            route, json.dumps({**valid_payload, "instance_id": 7}).encode()
        )
        malformed_key = request(
            ...
            json.dumps(
                {
                    **valid_payload,
                    "instance_id": f"instance-bad-{tag}",
                    "client_public_key": "invalid",
                }
            ).encode(),
        )
        if wrong_type["status"] < 400 or malformed_key["status"] < 400:
            raise AssertionError(
        ...
        repeat = request(route, json.dumps(valid_payload).encode())
        require_response(repeat, contract["response_fields"])
        health = request(base + "/Debug.Info", b"{}")
        if health["status"] != 200:
            raise AssertionError("Debug.Info was unavailable after registration")
        record(
        ...
        steps.append(
            {
                "id": step,
                "status": "PASS",
                "observed": "Protobuf returned every top-level field, repeat registration succeeded, and Debug.Info remained available.",
            }
        )
        print(f"STEP {step} END - PASS", flush=True)
        summary = "Debug.RegisterCvm passed its isolated JSON/protobuf, invalid-input, repeat, response-structure, and availability matrix."
    except Exception as error:  # noqa: BLE001
        status = "FAIL"
        failure = str(error)
        summary = f"Debug.RegisterCvm failed: {failure}"
    ...
    atomic_json(
        result_dir / "result.json",
        {
            ...
            "steps": steps,
            "artifacts": artifacts,
            "remarks": "All registrations are lease-owned; secret-bearing response values are reduced to structural metadata before persistence.",
        },
    )
