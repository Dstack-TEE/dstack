...
Verify that every method `Gateway` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the gateway's public RPC listener is reachable.
...
whose `service` is `Gateway`, and the vector set in
...
Resolve the gateway's public RPC listener from the lease-owned fixture, load the inventory
entries for `Gateway`, and confirm every one of them carries a policy.
