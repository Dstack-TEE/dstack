...
<a id="tc-gw-contract-003"></a>
# TC-GW-CONTRACT-003: Admin request-contract matrix
...
- Automation: Yes
- Requirements: [req-gw-contract-003](../../../../catalog/feature-audit.md#req-gw-contract-003)
- Risks: [risk-gw-contract-003](../../../../catalog/feature-audit.md#risk-gw-contract-003)
...
Verify that every `Admin` method handles absent, valid, boundary-invalid,
wrong-typed, unknown-field and malformed-framing input, in JSON and protobuf,
without breaking the invariants below.

## Preconditions

1. The gateway's authenticated admin listener is reachable on a lease-owned fixture. The matrix drives
   mutating methods with run-scoped identifiers, so it must never target a
   shared deployment.

## Test Data

The `gateway` entries in `api-inventory.json` whose `service` is `Admin`,
...
<a id="tc-gw-contract-003-step-01"></a>
### Step 1: Resolve the listener and the indexed contract

Resolve the listener from the fixture and load the `Admin` inventory entries.

**Expected results:**

- The listener is reachable and every declared method has a policy.

<a id="tc-gw-contract-003-step-02"></a>
...
- **L7** A method segment carrying a percent-encoded line break followed by a
  marker (`<Method>%0A<marker> INFO forged`) is logged, but no line of the
  Gateway log begins with the marker (PR #1401).

<a id="tc-gw-contract-003-step-03"></a>
