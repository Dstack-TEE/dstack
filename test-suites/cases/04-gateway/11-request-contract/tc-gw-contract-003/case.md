...
Verify that every method `Admin` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the gateway's authenticated admin listener is reachable.
...
The `gateway` entries in [`api-inventory.json`](../../../../catalog/api-inventory.json)
whose `service` is `Admin`, and the vector set in
...
Resolve the gateway's authenticated admin listener from the lease-owned fixture, load the inventory
entries for `Admin`, and confirm every one of them carries a policy.
