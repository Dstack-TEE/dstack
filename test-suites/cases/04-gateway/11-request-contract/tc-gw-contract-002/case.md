...
Verify that every method `Debug` declares answers absent, default, valid,
...
1. The shared plan prerequisites are healthy and the gateway's debug listener is reachable.
...
whose `service` is `Debug`, and the vector set in
...
Resolve the gateway's debug listener from the lease-owned fixture, load the inventory
entries for `Debug`, and confirm every one of them carries a policy.
