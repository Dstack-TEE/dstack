...
<a id="tc-gw-contract-002"></a>
# TC-GW-CONTRACT-002: Debug request-contract matrix
...
- Automation: Yes
- Requirements: [req-gw-contract-002](../../../../catalog/feature-audit.md#req-gw-contract-002)
- Risks: [risk-gw-contract-002](../../../../catalog/feature-audit.md#risk-gw-contract-002)
...
Verify that every `Debug` method handles absent, valid, boundary-invalid,
wrong-typed, unknown-field and malformed-framing input, in JSON and protobuf,
without breaking the invariants below.

## Preconditions

1. The gateway's debug listener is reachable on a lease-owned fixture. The matrix drives
   mutating methods with run-scoped identifiers, so it must never target a
   shared deployment.

## Test Data

The `gateway` entries in `api-inventory.json` whose `service` is `Debug`,
...
<a id="tc-gw-contract-002-step-01"></a>
### Step 1: Resolve the listener and the indexed contract

Resolve the listener from the fixture and load the `Debug` inventory entries.

**Expected results:**

- The listener is reachable and every declared method has a policy.

<a id="tc-gw-contract-002-step-02"></a>
...
<a id="tc-gw-contract-002-step-03"></a>
