...
### Fixed
- verifier/kms/gateway: issuer certificates and CRLs named by a GCP TPM or AWS Nitro certificate are fetched only from an allowlist of hosts, including across redirects. Those URLs come from certificates checked only after the fetch, so an unauthenticated `/verify` caller could make the verifier issue requests to any host, including internal ones. The default allows the Google and AWS hosts that publish this collateral; `attestation.allowed_collateral_hosts` replaces it, and `*` matches within one DNS label
