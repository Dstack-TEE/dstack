...
- guest-agent: `AttestGpu` runs NVIDIA GPU attestation on demand against a caller-supplied 32-byte nonce, so an application can re-check the device after a driver reload leaves a GPU that answers NVML but can no longer attest. It is a local liveness check, not a remote claim: an NVIDIA report binds the device and nonce but not the TD, so it is relayable until TDISP/TEE-IO. Serialised and rate-limited. Exposed in the Rust, Python, Go and JS SDKs
