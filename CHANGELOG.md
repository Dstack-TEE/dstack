...
### Removed
- sdk: the local `verify_signature` and `verify_signature_chain` helpers added earlier in this cycle are gone again, along with their committed cross-language vectors (`sdk/tests/vectors/signature_chain.json`) and the generator behind them. Neither ever shipped in a release. An SDK mirrors an API surface; verifying a chain is the relying party's job, and it needs no client and no connection -- `docs/guest-api-v1.md` specifies the rules normatively, down to the trust anchor, which is what a verifier should implement against rather than four hand-ports of the same byte format. `DstackClientV0.verify()` remains for single signatures on the frozen surface, since that is what the v0 surface offers
