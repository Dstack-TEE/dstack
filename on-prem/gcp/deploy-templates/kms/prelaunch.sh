...
# the only app deployment file it writes is .env.
set -eu

REGISTRY_RE='^[a-z0-9]([a-z0-9.-]*[a-z0-9])?(:[0-9]+)?(/[a-z0-9._-]+)+$'
SWP_PROXY_RE='^([0-9]{1,3}\.){3}[0-9]{1,3}:[0-9]{1,5}$'
...
SWP_PROXY="$(config_lookup SWP_PROXY swp_proxy || true)"
[ -n "$SWP_PROXY" ] || SWP_PROXY="$(md instance/attributes/swp-proxy)"

require_value DSTACK_REGISTRY "$DSTACK_REGISTRY"
require_value SWP_PROXY "$SWP_PROXY"
validate DSTACK_REGISTRY "$DSTACK_REGISTRY" "$REGISTRY_RE"
validate SWP_PROXY "$SWP_PROXY" "$SWP_PROXY_RE"
