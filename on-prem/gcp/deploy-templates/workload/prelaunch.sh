...
KMS_HOST="$(config_lookup KMS_HOST kms_host || true)"
[ -n "$KMS_HOST" ] || KMS_HOST="$(md instance/attributes/kms-host)"

require_value DSTACK_REGISTRY "$DSTACK_REGISTRY"
require_value KMS_HOST "$KMS_HOST"
validate DSTACK_REGISTRY "$DSTACK_REGISTRY" "$REGISTRY_RE"
validate KMS_HOST "$KMS_HOST" "$KMS_HOST_RE"
