...
fn platform_from_legacy_quote(quote: AttestationQuote) -> PlatformEvidence {
    match quote {
        ...
        AttestationQuote::DstackAmdSevSnp(SnpQuote { report, cert_chain }) => {
...
impl AttestationMode {
    ...
    pub fn is_composable(&self) -> bool {
        match self {
            Self::DstackTdx => true,
            // SEV-SNP launch measurement does not provide a TDX RTMR3-equivalent
            // runtime event extension path yet, so runtime events are
            // informational until an SNP-specific app binding is added.
            Self::DstackAmdSevSnp => false,
            Self::DstackGcpTdx => true,
            Self::DstackNitroEnclave => false,
        }
...
mod compatibility_tests {
    ...
    fn attestation_quote_scale_discriminants_preserve_existing_wire_values() {
        assert_eq!(AttestationQuote::DstackGcpTdx.encode(), vec![1]);
        assert_eq!(AttestationQuote::DstackNitroEnclave.encode(), vec![2]);
    ...
    pub fn decode_app_info_ex(&self, boottime_mr: bool, vm_config: &str) -> Result<AppInfo> {
        ...
        let mrs = match &self.quote {
            ...
            | AttestationQuote::DstackNitroEnclave => {
                bail!("unsupported attestation quote for app info decoding");
            }
        };
...
impl Attestation {
    ...
    pub fn quote_with_app_id(report_data: &[u8; 64], app_id: Option<[u8; 20]>) -> Result<Self> {
        ...
        let config = match &quote {
            AttestationQuote::DstackAmdSevSnp(_) | AttestationQuote::DstackTdx(_) => {
