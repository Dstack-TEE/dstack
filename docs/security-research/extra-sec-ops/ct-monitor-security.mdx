...
`ct_monitor` watches for new certificates issued for dstack domains by querying crt.sh and checks them against the expected public keys from the dstack-gateway. It sends alerts if it finds any unauthorized or suspicious certificates.
