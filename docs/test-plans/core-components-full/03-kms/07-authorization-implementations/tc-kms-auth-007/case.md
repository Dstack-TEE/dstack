...
# TC-KMS-AUTH-007: Ethereum RPC failure reorg and finality handling
...
Verify ethereum rpc failure reorg and finality handling with explicit success, boundary, failure, restart, and isolation observations.
...
Inject timeout, malformed response, wrong chain, stale head, short/deep reorg, and delayed confirmations around authorization.

**Expected results:**

- Authorization fails closed when state is uncertain, waits configured finality, invalidates reorged cache entries, and recovers on canonical state.
