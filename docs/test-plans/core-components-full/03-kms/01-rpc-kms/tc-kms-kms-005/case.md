...
- Prepared RPC contract: `KMS.GetTempCaCert` takes `google.protobuf.Empty` (no fields) and returns `GetTempCaCertResponse` (`temp_ca_cert: string`, `temp_ca_key: string`, `ca_cert: string`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
