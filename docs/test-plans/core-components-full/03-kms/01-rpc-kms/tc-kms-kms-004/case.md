...
<a id="tc-kms-kms-004"></a>
# TC-KMS-KMS-004: KMS.GetMeta
...
- Automation: Yes
- Requirements: [req-kms-kms-004](../../../feature-audit.md#req-kms-kms-004)
- Risks: [risk-kms-kms-004](../../../feature-audit.md#risk-kms-kms-004)
- Source: `dstack/kms/rpc/proto/kms_rpc.proto:103`
...
- Prepared RPC contract: `KMS.GetMeta` takes `google.protobuf.Empty` (no fields) and returns `GetMetaResponse` (`ca_cert: string`, `allow_any_upgrade: bool`, `k256_pubkey: bytes`, `bootstrap_info: BootstrapResponse`, `is_dev: bool`, `gateway_app_id: string`, `kms_contract_address: string`, `chain_id: uint64`, `app_auth_implementation: string`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
...
Verify the complete request, response, authorization, state transition, and error contract of `KMS.GetMeta`.
...
- The valid call returns `GetMetaResponse` with every documented field and exhibits the documented `GetMeta` state and side effects; extraneous or malformed bodies are ignored without changing the response or state, invalid routing returns a structured transport error, and protected calls reject missing credentials.

<a id="tc-kms-kms-004-step-03"></a>
