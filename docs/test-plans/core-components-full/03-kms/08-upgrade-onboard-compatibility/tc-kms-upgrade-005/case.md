...
# TC-KMS-UPGRADE-005: Old source rejects 0.6.0 target in TDX-lite mode
...
Prove legacy-only source KMS nodes cannot accidentally authorize a lite-attestation target.
...
For 0.5.4, 0.5.8, and 0.5.11 sources, boot an otherwise authorized 0.6.0 target with `tdx_attestation_variant=lite` (and separately `auto` resolving to lite) and attempt onboard.

**Expected results:**

- Every old source rejects the target before root transfer due to unreproducible legacy measurements; no fallback or verification-disable path is used.
