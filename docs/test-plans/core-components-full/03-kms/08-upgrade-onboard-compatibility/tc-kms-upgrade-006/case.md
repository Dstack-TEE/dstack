...
# TC-KMS-UPGRADE-006: Legacy mode is forced throughout mixed-source cutover
...
Verify VMM and persisted target configuration cannot silently resolve `auto` to lite while any old source remains.
...
- The approved upgrade deployment explicitly records legacy end to end; auto/lite variants are detected and blocked from cutover while a pre-0.6 source verifies peers.
