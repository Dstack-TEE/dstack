...
Prove 0.5.4 bridge pre-flight uses its QEMU 9.1.50-era ACPI-table computation rather than the 0.6.0 tool.
...
Run `dstack-mr diagnose` on the same 0.5.4/bridge quote and vm_config with age-matched and 0.6.0 `dstack-acpi-tables`.

**Expected results:**

- The age-matched tool reports RTMR0 MATCH; the incompatible tool exposes the first divergent ACPI event and is not accepted as upgrade evidence.
