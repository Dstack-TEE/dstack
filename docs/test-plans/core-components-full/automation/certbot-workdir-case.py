...
def main() -> int:
    ...
    checks = {
        "candidate_tests_passed": completed.returncode == 0 and passed_count >= 3,
        "path_contract_exercised": "paths_and_archive_listing_are_stable" in output,
    ...
    observed = (
        "Candidate workdir path, ordered archive, rollback, malformed-state isolation, and cleanup checks passed."
        if passed
