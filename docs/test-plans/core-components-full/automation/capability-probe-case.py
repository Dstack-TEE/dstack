...
def required_capability(plan_root: pathlib.Path, case_id: str) -> str:
    ...
    index = json.loads((plan_root / "index.json").read_text(encoding="utf-8"))
    profiles = json.loads(
        (plan_root / "fixtures" / "profiles.json").read_text(encoding="utf-8")
    )["profiles"]
    for chapter in index["chapters"]:
        for section in chapter["sections"]:
            for case in section.get("cases", []):
                if case["id"] != case_id:
                    continue
                name = str((case.get("fixture") or {}).get("profile", ""))
                for capability in profiles.get(name, {}).get(
                    "required_capabilities", []
                ):
                    if capability in PROBES:
                        return capability
                raise SystemExit(
                    f"{case_id} profile {name!r} names no probeable capability"
                )
