...
def limits(pid: int) -> tuple[int, int]:
    ...
    match = re.search(r"^Max open files\s+(\d+)\s+(\d+)\s+files\s*$", text, re.MULTILINE)
    if match is None:
        raise AssertionError("Max open files limit was unavailable")
    return int(match.group(1)), int(match.group(2))


def start(binary: str, config: pathlib.Path, environment: dict[str, str]) -> subprocess.Popen[bytes]:
    """Start a case-owned candidate without retaining native output."""
    return subprocess.Popen(
        [binary, "--config", str(config)], stdout=subprocess.DEVNULL,
...
def main() -> int:
    ...
    guest_socket = str(values["gateway_guest_simulator"]["services"]["DstackGuest"]["socket"])
    ...
    try:
        ...
        startup_checks = {
            ...
            "debug_listener_disabled": core["debug"]["insecure_enable_debug_rpc"] is False,
            ...
            "crypto_provider_explicit": core["proxy"]["tls_crypto_provider"] in {"aws-lc-rs", "ring"},
            "private_files_exist": all(path.is_file() and path.stat().st_size > 0 for path in private_files),
        ...
        if conflict_code is None:
            stop(conflict)
        conflict_rejected = conflict_code not in {None, 0} and health(health_url, token) == 200
        ...
        invalid_config.write_text(
            invalid_text.replace(marker, f'cert_chain = "{workspace / "data/absent-cert.pem"}"', 1)
        )
        invalid = start(binary, invalid_config, environment)
        invalid_code = wait_exit(invalid)
        if invalid_code is None:
            stop(invalid)
        invalid_rejected = invalid_code not in {None, 0} and health(health_url, token) == 200
        ...
        restart_soft, restart_hard = limits(restarted.pid) if restart_healthy else (0, 0)
        checks = {
            **startup_checks,
            ...
            "restart_ulimit_raised": restart_soft == restart_hard and restart_soft == hard,
        ...
        observation = {
            "candidate_commit": runtime["candidate_commit"], "checks": checks,
            "initial_open_file_soft": soft, "initial_open_file_hard": hard,
            "restart_open_file_soft": restart_soft, "restart_open_file_hard": restart_hard,
        ...
        artifacts.append({
            "path": "artifacts/gateway-production-startup.json", "step_id": f"{CASE_ID}-step-02",
