    ...
    try:
        ...
        payloads = [
            (
                ...
                "/run/dstack-test-vtpm/dstack-tee-simulator",
            ),
            (
                pathlib.Path(
                    runtime["prepared_binaries"]["dstack_util"]["path"]
                ).read_bytes(),
                "/run/dstack-test-vtpm/dstack-util",
            ),
            (
                (
                    repo
                    / "docs/test-plans/core-components-full/automation/vtpm-cli-mkosi.sh"
                ).read_bytes(),
                "/run/dstack-test-vtpm/run-case",
            ),
        ]
        for data, target in payloads:
            cp = run(
                [
                    *ssh,
                    f"mkdir -p /run/dstack-test-vtpm && install -m 0755 /dev/stdin {target}",
                ],
                data=data,
                timeout=180,
            )
        ...
        cp = run([*ssh, "/run/dstack-test-vtpm/run-case"], timeout=600)
        log = cp.stdout + cp.stderr
        (artifacts / "mkosi-vtpm.log").write_bytes(log)
        if cp.returncode:
            raise RuntimeError(
        ...
        if any(matrix.get(key) is not True for key in required_true):
            raise RuntimeError(f"unexpected matrix: {matrix}")
        status = "PASS"
        summary = "vTPM attest, TPM quote/verify, mutation, fault, retry, identity, and file-safety checks passed inside mkosi."
            ...
            evidence["cleanup_returncode"] = run(
                [
                    *ssh,
                    "bash",
                    "-lc",
                    "pkill -f /run/dstack-test-vtpm/dstack-tee-simulator 2>/dev/null || true; fusermount3 -uz /run/dstack-test-eventlog/report 2>/dev/null || true; rm -rf /run/dstack-test-eventlog /run/log/dstack",
                ],
                timeout=30,
            ).returncode
    evidence["duration_seconds"] = round(time.monotonic() - started, 3)
    write_json(artifacts / "vtpm-cli-mkosi.json", evidence)
    artifact = {
        "path": "artifacts/vtpm-cli-mkosi.json",
        "step_id": f"{CASE_ID}-step-01",
        "name": "mkosi vTPM CLI suite",
        "description": "Guest provenance and vTPM attest, quote, verify, mutation, fault, retry, identity, and cleanup evidence.",
    }
    write_json(artifacts / "manifest.json", {"artifacts": [artifact]})
    observed = (
        summary
        if status == "FAIL"
        else "The lease-owned mkosi guest passed the vTPM CLI trust, mutation, fault, retry, and isolation matrix using the GCP vTPM simulator ABI."
    )
    write_json(
        result / "result.json",
        {
            ...
            "artifacts": [artifact],
            "remarks": "Simulator execution inside mkosi proves CLI cryptographic behavior and fault handling; it does not prove vendor hardware isolation or vendor-signed certificates.",
        },
    )
    return 0
