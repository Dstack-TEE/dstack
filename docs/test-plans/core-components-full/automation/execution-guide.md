...
Before running cases, execute `automation/prepare-run.sh`. It writes a runtime
...
fixture_root=/path/to/dstack-test-fixtures/verifier/full-tdx-0.5.4.1
...
3. Install with:
   ...
   ```bash
   python3 automation/install-prepared-binary.py \
...
"$plan_root/automation/start-simulator.sh" \
...
"$plan_root/automation/stop-simulator.sh" \
  "$RESULT_DIR/artifacts/simulator-fixture.json"
```
