...
def request(url: str, *, token: str | None = None, post: bool = False) -> tuple[int, bytes]:
...
def main() -> int:
    ...
    try:
        ...
        steps = [
            {"id": f"{CASE_ID}-step-01", "status": "PASS", "observed": "The case-owned public and admin listeners were independently reachable."},
            {"id": f"{CASE_ID}-step-02", "status": "PASS", "observed": "Public, admin, and unknown-method routes matched their intended exposure boundaries; dashboard construction either completed or returned its bounded server error."},
            {"id": f"{CASE_ID}-step-03", "status": "PASS", "observed": "Admin methods remained unreachable through the public handler and malformed method selection returned a bounded error."},
            {"id": f"{CASE_ID}-step-04", "status": "PASS", "observed": "No bearer credential was present in any bounded response and the adjacent listener remained healthy."},
        ]
        status = "PASS"
