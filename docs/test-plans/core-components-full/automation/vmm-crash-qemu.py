...
def main() -> int:
    ...
    for pid in children:
        ...
        try:
            argv = [part.decode(errors="replace") for part in cmdline_path.read_bytes().split(b"\0") if part]
        except FileNotFoundError:
            continue
        executable = pathlib.Path(argv[0]).name if argv else ""
        if executable.startswith("qemu-system-") and any(owned_path in arg for arg in argv):
            matches.append(pid)
    if len(matches) != 1:
