...
"""Exercise dstack-util RA and key commands inside a mkosi guest."""
...
def main() -> int:
    """Execute the RA and key CLI matrix in the fixture-owned guest."""
    ...
    try:
        ...
        payloads = [
            (
                ...
                "/run/dstack-test-ra-key/dstack-tee-simulator",
            ),
            (
                pathlib.Path(
                    runtime["prepared_binaries"]["dstack_util"]["path"]
                ).read_bytes(),
                "/run/dstack-test-ra-key/dstack-util",
            ),
            (
                (
                    repo
                    / "docs/test-plans/core-components-full/automation/ra-key-cli-mkosi.sh"
                ).read_bytes(),
                "/run/dstack-test-ra-key/run-case",
            ),
        ]
        for data, target in payloads:
            cp = run(
                [
                    *ssh,
                    f"mkdir -p /run/dstack-test-ra-key && install -m 0755 /dev/stdin {target}",
                ],
                data=data,
                timeout=180,
            )
        ...
        cp = run([*ssh, "/run/dstack-test-ra-key/run-case"], timeout=600)
        log = cp.stdout + cp.stderr
        (artifacts / "mkosi-ra-key.log").write_bytes(log)
        if cp.returncode:
            raise RuntimeError(
                f"mkosi RA/key rc={cp.returncode}: {log.decode(errors='replace')[-1600:]}"
            )
        ...
        if (
            matrix.get("ca_levels") != [0, 1, 2]
            or matrix.get("private_modes") != "600"
            ...
            evidence["cleanup_returncode"] = run(
                [
                    *ssh,
                    "bash",
                    "-lc",
                    "pkill -f /run/dstack-test-ra-key/dstack-tee-simulator 2>/dev/null || true; fusermount3 -uz /run/dstack-test-ra-key/report 2>/dev/null || true; rm -rf /run/dstack-test-ra-key /run/log/dstack",
                ],
                timeout=30,
            ).returncode
    evidence["duration_seconds"] = round(time.monotonic() - started, 3)
    write_json(artifacts / "ra-key-cli-mkosi.json", evidence)
    artifact = {
    ...
    observed = (
        summary
        if status == "FAIL"
        else "The lease-owned mkosi guest passed all RA and key CLI modes and safety boundaries using the TDX simulator ABI."
    )
    write_json(
        result / "result.json",
        {
