...
"""Exercise dstack-util versioned attestation commands inside a mkosi guest."""
    ...
    try:
        ...
        payloads = [
            (
                ...
                "/run/dstack-test-attest/dstack-tee-simulator",
            ),
            (
                pathlib.Path(
                    runtime["prepared_binaries"]["dstack_util"]["path"]
                ).read_bytes(),
                "/run/dstack-test-attest/dstack-util",
            ),
            (
                (
                    repo
                    / "docs/test-plans/core-components-full/automation/versioned-attestation-mkosi.sh"
                ).read_bytes(),
                "/run/dstack-test-attest/run-case",
            ),
        ]
        for data, target in payloads:
            cp = run(
                [
                    *ssh,
                    f"mkdir -p /run/dstack-test-attest && install -m 0755 /dev/stdin {target}",
                ],
                data=data,
                timeout=180,
            )
        ...
        cp = run([*ssh, "/run/dstack-test-attest/run-case"], timeout=600)
        log = cp.stdout + cp.stderr
        (artifacts / "mkosi-versioned-attestation.log").write_bytes(log)
        if cp.returncode:
            raise RuntimeError(
                f"mkosi attestation rc={cp.returncode}: {log.decode(errors='replace')[-1600:]}"
            )
        ...
        status = "PASS"
        summary = "Versioned attestation V0/V1, boundary, strip, malformed-input, fault, retry, identity, and file-safety checks passed inside mkosi."
            ...
            evidence["cleanup_returncode"] = run(
                [
                    *ssh,
                    "bash",
                    "-lc",
                    "pkill -f /run/dstack-test-attest/dstack-tee-simulator 2>/dev/null || true; fusermount3 -uz /run/dstack-test-attest/report 2>/dev/null || true; rm -rf /run/dstack-test-attest /run/log/dstack",
                ],
                timeout=30,
            ).returncode
    evidence["duration_seconds"] = round(time.monotonic() - started, 3)
    write_json(artifacts / "versioned-attestation-mkosi.json", evidence)
    artifact = {
        "path": "artifacts/versioned-attestation-mkosi.json",
        "step_id": f"{CASE_ID}-step-01",
        "name": "mkosi versioned-attestation CLI suite",
        "description": "Guest provenance and versioned encoding, boundary, strip, fault, retry, identity, and cleanup evidence.",
    }
    write_json(artifacts / "manifest.json", {"artifacts": [artifact]})
    observed = (
        summary
        if status == "FAIL"
        else "The lease-owned mkosi guest passed all versioned attestation CLI modes and safety boundaries using the TDX simulator ABI."
    )
    write_json(
        result / "result.json",
        {
            ...
            "artifacts": [artifact],
            "remarks": "Simulator execution proves functional encoding, binding, errors, identity, and file safety; it does not prove physical TDX isolation or vendor-signed evidence.",
        },
    )
    return 0
