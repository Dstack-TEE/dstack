...
tools/dstack-test/dstack-test run-plan --plan <plan> --driver program

# Fast regression over every case that owns a checked-in harness.
tools/dstack-test/dstack-test sweep --plan <plan> --run-id <id> --workers 8 \
...
- `automation/mine-passing-attempt.py` refuses to emit a spec when it cannot
...
`automation/`. Extending a table is cheaper and more reviewable than writing a
new script:
...
| replay of a mined attempt | `replay-case.py` + `automation/replay/<case>.json` |
...
them in `automation/tdxlab-manifest.json`, which `prepare-run.sh` merges into
...
361 cases total, 89 scripted. Of the 272 remaining:

- **31 are RPC-shaped** — the method appears in `api-inventory.json`, so the
  work is extending a table with a payload and expectations. Several still need
  real attestation input: `KMS.GetAppKey`, `KMS.GetKmsKey` and `KMS.SignCert`
  take a `vm_config`, which needs a running CVM.
- **241 are prose behavioural cases** — `actions_under_test` is a sentence such
  as "Encrypted root/data volume provisioning", not a method. Each needs a
  purpose-written harness derived from its `case.md`. There is no mechanical
  shortcut: the recorded evidence from the agent-driven run is prose
  observation, not replayable operations.

The largest prose clusters are `compatibility-matrix` (39), `gateway-cluster`
(39), `vmm-empty-control-plane` (27) and `kms-ready` (26). Working a whole
fixture profile at a time amortises the setup understanding across its cases.
...
on whether the case owns an entrypoint (tools/dstack-test/dstack-test, around
line 1126):
...
So running the full plan today still spawns an agent for each of the 268 cases
that have no harness. The claim is exact only for the 93 scripted cases.

Two consequences worth keeping in mind:

- Quote the scripted count alongside any "programmatic" claim. "93 of 361 cases
  run deterministically" is true; "the plan runs without AI" is not, yet.
- `run-plan --driver=program --require-script` refuses the first case without
  ...
  rather than conventional. Verified end to end: it executed 13 scripted cases,
  halted at `tc-gos-dstackguest-008` with `driver.unscripted`, and produced no
  `orchestrator.jsonl`, so no agent was ever spawned.
