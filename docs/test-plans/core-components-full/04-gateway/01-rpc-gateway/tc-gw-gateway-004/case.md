...
- Prepared RPC contract: `Gateway.GetPeers` takes `google.protobuf.Empty` (no fields) and returns `GetPeersResponse` (`my_id: uint32`, `my_url: string`, `peers: PeerInfo`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
