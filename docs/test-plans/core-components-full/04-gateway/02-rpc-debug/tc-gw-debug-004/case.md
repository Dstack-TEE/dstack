...
- Prepared RPC contract: `Debug.GetProxyState` takes `google.protobuf.Empty` (no fields) and returns `DebugProxyStateResponse` (`instances: ProxyStateInstance`, `allocated_addresses: string`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
