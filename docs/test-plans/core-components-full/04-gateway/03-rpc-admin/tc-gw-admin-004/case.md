...
- Prepared RPC contract: `Admin.RenewCert` takes `google.protobuf.Empty` (no fields) and returns `RenewCertResponse` (`renewed: bool`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
