...
- Prepared RPC contract: `Admin.GetDefaultDnsCredential` takes `google.protobuf.Empty` (no fields) and returns `GetDefaultDnsCredentialResponse` (`default_id: string`, `credential: DnsCredentialInfo`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
