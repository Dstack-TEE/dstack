...
- Prepared RPC contract: `Admin.GetNodeStatuses` takes `google.protobuf.Empty` (no fields) and returns `GetNodeStatusesResponse` (`statuses: NodeStatusEntry`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
