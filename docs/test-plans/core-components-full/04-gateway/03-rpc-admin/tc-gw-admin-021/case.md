...
- Prepared RPC contract: `Admin.ListZtDomains` takes `google.protobuf.Empty` (no fields) and returns `ListZtDomainsResponse` (`domains: ZtDomainInfo`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
