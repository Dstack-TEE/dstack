...
- Prepared RPC contract: `Admin.GetGlobalConnections` takes `google.protobuf.Empty` (no fields) and returns `GlobalConnectionsStats` (`total_connections: uint64`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
