...
- Prepared RPC contract: `Admin.WaveKvStatus` takes `google.protobuf.Empty` (no fields) and returns `WaveKvStatusResponse` (`enabled: bool`, `persistent: StoreSyncStatus`, `ephemeral: StoreSyncStatus`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
