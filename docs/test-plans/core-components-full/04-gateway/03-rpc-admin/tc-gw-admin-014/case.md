...
- Prepared RPC contract: `Admin.ListDnsCredentials` takes `google.protobuf.Empty` (no fields) and returns `ListDnsCredentialsResponse` (`credentials: DnsCredentialInfo`, `default_id: string`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
