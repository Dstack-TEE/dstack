...
- Prepared RPC contract: `DstackGuest.GpuInfo` takes `google.protobuf.Empty` (no fields) and returns `GpuInfoResponse` (`attestation: string`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
