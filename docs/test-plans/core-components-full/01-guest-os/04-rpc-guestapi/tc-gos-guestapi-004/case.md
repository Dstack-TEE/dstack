...
- Prepared RPC contract: `GuestApi.ListContainers` takes `google.protobuf.Empty` (no fields) and returns `ListContainersResponse` (`containers: Container`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
