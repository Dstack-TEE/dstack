...
- Prepared RPC contract: `Vmm.ListGpus` takes `google.protobuf.Empty` (no fields) and returns `ListGpusResponse` (`gpus: GpuInfo`, `allow_attach_all: bool`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
