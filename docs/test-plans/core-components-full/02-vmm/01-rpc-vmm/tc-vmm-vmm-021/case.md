...
- Prepared RPC contract: `Vmm.ListRegistryImages` takes `google.protobuf.Empty` (no fields) and returns `RegistryImageListResponse` (`images: RegistryImageInfo`). The authoritative field matrix is the matching entry in [`api-inventory.json`](../../../api-inventory.json); do not reconstruct it from implementation source.
