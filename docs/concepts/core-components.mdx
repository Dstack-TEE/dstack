...
dstack consists of five core components working together to provide confidential computing:

```mermaid
graph TB
    subgraph "Host Environment"
        VMM["🔧 VMM<br/>(Virtual Machine Manager)"]
        Gateway["🌐 Gateway<br/>(Network Proxy)"]
        KMS["🔐 KMS<br/>(Key Management)"]
    end
    
    subgraph "Confidential VM"
        GuestOS["💽 Guest OS<br/>(meta-dstack Image)"]
        DstackUtil["⚙️ dstack-util<br/>(TDX Utilities)"]
        Agent["🤖 Guest Agent<br/>(Runtime Services)"]
    end
    
    VMM -->|"Boots & Manages"| GuestOS
    Gateway -->|"Routes Traffic"| Agent
    KMS -->|"Provides Keys"| Agent
    GuestOS -->|"Contains"| DstackUtil
    GuestOS -->|"Runs"| Agent
    
    Agent -->|"Registers via gRPC"| Gateway
    Agent -->|"Requests Keys via RA-TLS"| KMS
    VMM -->|"Configures"| Gateway
```
...
<table className="w-full my-4 text-sm">
  ...
  <tbody>
    <tr>
      ...
      <td className="p-2">Configuration</td>
      <td className="p-2">VM Lifecycle</td>
    </tr>
    <tr className="bg-gray-50">
      <td className="p-2 font-semibold">Gateway</td>
      <td className="p-2">Status Updates</td>
      <td className="p-2 bg-gray-100">-</td>
      <td className="p-2">Certificate Requests</td>
      <td className="p-2">WireGuard + gRPC</td>
    </tr>
    <tr>
      <td className="p-2 font-semibold">KMS</td>
      <td className="p-2">-</td>
      <td className="p-2">Certificate Provision</td>
      <td className="p-2 bg-gray-100">-</td>
      <td className="p-2">RA-TLS Key Provision</td>
    </tr>
    <tr className="bg-gray-50">
      <td className="p-2 font-semibold">Guest Agent</td>
      <td className="p-2">Health/Status</td>
      <td className="p-2">gRPC Registration</td>
      <td className="p-2">RA-TLS Key Requests</td>
      <td className="p-2 bg-gray-100">-</td>
    </tr>
  </tbody>
</table>
