...
- **Cryptographic Services**: Diffie-Hellman key agreement and AES-GCM encryption/decryption [4](#0-3) 
...
- **TDX Report Generation**: Creates attestation reports from 64-byte report data [6](#0-5) 
- **TDX Quote Generation**: Produces signed quotes for remote attestation [7](#0-6) 
- **RTMR Extension Operations**: Extends Runtime Measurement Registers with application events [8](#0-7) 
- **Measurement Display**: Shows current RTMR state and decoded measurement information [9](#0-8) 
...
- **Encryption Configuration**: Uses AES-XTS-Plain64 cipher with PBKDF2 key derivation [10](#0-9) 
- **ZFS Integration**: Creates encrypted ZFS pools with Blake3 checksums and autoexpand capabilities [11](#0-10) 
- **Mount Management**: Handles both new initialization and existing disk mounting [12](#0-11) 

#### Key Management Integration
- **KMS-based Key Retrieval**: Supports remote key management services with attestation [13](#0-12) 
- **Local Key Provider**: SGX-based key derivation for isolated environments [14](#0-13) 
- **Key Provider Verification**: Validates and measures key providers for security [15](#0-14) 

### 3. Cryptographic Services

Advanced cryptographic operations including:

- **Certificate Generation**: Creates Remote Attestation TLS certificates with TDX quotes [16](#0-15) 
- **CA Certificate Creation**: Generates Certificate Authority certificates with attestation [17](#0-16) 
- **Application Key Management**: Produces complete application key sets with quotes [18](#0-17) 
- **Diffie-Hellman Operations**: X25519 key agreement and AES-GCM decryption [19](#0-18) 
...
- **Boot Process Measurement**: Tracks system initialization stages through RTMR extensions [23](#0-22) 
...
- **Configuration Generation**: Creates agent.json with disk paths and PCCS URL [25](#0-24) 
...
- **WireGuard VPN Setup**: Configures secure tunnels with TDX-attested certificates [26](#0-25) 
- **Certificate-based Authentication**: Uses Remote Attestation TLS for gateway registration [27](#0-26) 
- **Firewall Configuration**: Sets up iptables rules for secure WireGuard operation [28](#0-27) 

### Host Communication

Provides secure bidirectional communication with the host system:

- **Event Notification**: Sends structured events and payloads to host API [29](#0-28) 
...
```mermaid
graph TD
    A["System Boot"] --> B["Load Host Shared Files"]
    B --> C["Measure App Composition"]
    C --> D["Request App Keys"]
    D --> E["Setup Disk Encryption"]
    E --> F["Mount Data Disk"]
    F --> G["Configure Network"]
    G --> H["Setup Docker"]
    H --> I["System Ready"]
```
...
```mermaid
graph TD
    A["Determine Key Provider"] --> B{"Provider Type"}
    B -->|KMS| C["Connect to KMS"]
    B -->|Local| D["Get Sealing Key"]
    B -->|None| E["Generate Temp Keys"]
    C --> F["Validate Certificate"]
    D --> G["Derive from SGX"]
    E --> H["Random Generation"]
    F --> I["Decrypt App Keys"]
    G --> I
    H --> I
    I --> J["Verify Provider ID"]
    J --> K["Measure Provider"]
```
...
- **tdx-attest**: Low-level TDX operations and RTMR management [30](#0-29) 
- **ra-tls**: Remote Attestation TLS certificate generation [31](#0-30) 
- **dstack-kms-rpc**: Key Management Service communication [32](#0-31) 
- **dstack-gateway-rpc**: Gateway registration and WireGuard setup [33](#0-32) 
- **host-api**: Host system communication and event reporting [34](#0-33) 
...
dstack-util is a critical component of the dstack system that bridges the gap between TEE hardware capabilities and the application layer. It handles the complex setup procedures required for secure computing in trusted execution environments, including disk encryption with LUKS, network setup with WireGuard, and integration with key management services. The tool is designed to be called both during system initialization and application runtime to maintain security and manage the secure environment.
