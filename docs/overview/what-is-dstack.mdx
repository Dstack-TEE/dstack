...
This page gives you a high-level overview of dstack's architecture—a developer-friendly SDK designed for secure deployment of containerized applications in Trusted Execution Environments (TEEs). By leveraging Intel TDX (Trust Domain Extensions), dstack enables confidential containers that keep your data and code protected through hardware-based isolation. Here, you'll get a look at the platform's main components, how they work together, and the security model that underpins dstack.

## Introduction

For a detailed overview, see the [dstack README](https://github.com/Dstack-TEE/dstack/blob/45ebd05a/README.md).

dstack is a developer-friendly, security-first SDK that streamlines the deployment of containerized applications into Trusted Execution Environments (TEEs). By leveraging Intel TDX, dstack creates Confidential Virtual Machines (CVMs) that provide hardware-based isolation for sensitive workloads. Developers can use familiar tools like `docker-compose.yaml` files, while dstack automatically manages the complexities of TEE deployment, key management, and secure networking.
...
This corrected version removes several inaccuracies from the original document:

1. **TEE Support**: The original document claimed support for AMD SEV-SNP and ARM CCA, but dstack currently only supports Intel TDX [18](#0-17) .

2. **Component Names**: The original mentioned "dstack-os" and "dstack-ingress" which don't exist in the actual codebase. The real components are dstack-vmm, dstack-gateway, dstack-kms, dstack-guest-agent, and supervisor.

3. **Architecture Claims**: Removed speculative claims about "Portable Confidential Containers, Decentralized Code Management, and Verifiable Domain Management" as core innovations, as these terms don't appear in the actual codebase.

4. **Technical Details**: Corrected technical implementation details to match the actual codebase rather than speculative descriptions.

5. **Focus**: Adjusted the emphasis from "Web3-first" to accurately reflect dstack's broader confidential computing focus while maintaining its blockchain integration aspects.
