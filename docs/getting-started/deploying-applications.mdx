...
- ✅ **TDX-capable hardware** with Intel TDX support [1](#0-0) 
- ✅ **dstack-vmm running** (Virtual Machine Manager) [2](#0-1) 
- ✅ **Guest OS images downloaded** (dstack OS images) [3](#0-2) 
- ✅ **KMS and Gateway deployed** (for production workloads) [4](#0-3) 
...
This guide reflects the actual dstack architecture based on the codebase. The deployment process differs significantly from generic container orchestration platforms due to dstack's focus on confidential computing and hardware-based security guarantees. The system requires on-chain registration and uses specialized tooling (vmm-cli.py) rather than standard Docker or Kubernetes commands.

For development and testing, you can use simpler configurations without KMS or on-chain registration. Production deployments require the full security stack including KMS, Gateway, and blockchain-based application authorization.
