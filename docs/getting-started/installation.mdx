...
This comprehensive guide walks you through installing and configuring dstack on **Linux systems only**. Self-hosted dstack requires Linux due to its deep integration with kernel-level security features, container runtimes, and TEE hardware management.

<div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
  <div className="flex">
    <div className="ml-3">
      <p className="text-sm text-blue-700">
        <strong>Alternative:</strong> If you don't have TEE hardware or prefer managed infrastructure, use <a href="https://cloud.phala.network/register?invite=beta" className="underline">Phala Cloud</a> to test dstack without local installation.
      </p>
    </div>
  </div>
</div>
...
**dstack self-hosting is exclusively supported on Linux.** The platform requires:
- Direct kernel module access for TEE hardware (Intel TDX)
- Container runtime integration at the kernel level  
- Network namespace manipulation for secure networking
...
dstack requires TEE-capable hardware for production security guarantees:
...
<details>
<summary>How to verify Intel TDX support on your Linux system</summary>

Check if your CPU and kernel support TDX:

```bash
# Check CPU TDX capabilities
grep -E 'tdx|TDX' /proc/cpuinfo
```

If you see TDX-related output, your CPU supports it. For kernel support verification:

```bash
# Check TDX kernel module availability
sudo dmesg | grep -i tdx
```

If TDX is properly enabled, you should see initialization messages.

</details>

### Network Requirements

Ensure these ports are available on your Linux system:
